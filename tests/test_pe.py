...
class PETest(unittest.TestCase):
    ...
    def test_pe_win_x86_clipboard(self):
        ql = Qiling(
            ["../examples/rootfs/x8664_windows/bin//x8664_clipboard_test.exe"], "../examples/rootfs/x8664_windows"
        )
        ql.run()
        del ql
    ...
    def test_pe_win_x86_NtQueryInformationSystem(self):
        ql = Qiling(
            ["../examples/rootfs/x86_windows/bin/NtQuerySystemInformation.exe"], "../examples/rootfs/x86_windows"
        )
        ql.run()
        del ql
    ...
    def test_pe_win_x86_argv(self):
        def check_print(ql: Qiling, address: int, params):
            ...
            params = ql.os.resolve_fcall_params(
                {"_Options": PARAM_INT64, "_Stream": POINTER, "_Format": STRING, "_Locale": DWORD, "_ArgList": POINTER}
            )
