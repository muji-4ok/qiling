...
def handle_bnj(ql, cur_addr):
    return {QL_ARCH.MIPS: handle_bnj_mips, QL_ARCH.ARM: handle_bnj_arm, QL_ARCH.ARM_THUMB: handle_bnj_arm,}.get(
        ql.archtype
    )(ql, cur_addr)
...
def handle_bnj_mips(ql, cur_addr):
    ...
    if line.mnemonic.startswith("j") or line.mnemonic.startswith("b"):
        ...
        targets = [read_reg_val(each) if "$" in each else parse_int(each) for each in line.op_str.split(", ")]
