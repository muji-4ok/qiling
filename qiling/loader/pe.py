    ...
    def load_dll(self, name: bytes, driver: bool = False) -> int:
        ...
        if not cached or not loaded:
            ...
            if (image_base + image_size) > self.ql.mem.max_mem_addr:
                ...
                self.ql.log.debug(
                    f"DLL preferred base address exceeds memory upper bound, loading to: {image_base:#x}"
                )
...
class QlLoaderPE(QlLoader, Process):
    ...
    def run(self):
        ...
        self.ql.os.heap = QlMemoryHeap(
            self.ql, self.ql.os.heap_base_address, self.ql.os.heap_base_address + self.ql.os.heap_base_size
        )
    ...
    def load(self):
        ...
        if self.path and not self.ql.code:
            ...
            self.entry_point = self.pe_entry_point = (
                self.pe_image_address + self.pe.OPTIONAL_HEADER.AddressOfEntryPoint
            )
            ...
            self.images.append(
                Image(
                    ...
                    self.pe_image_address + self.pe.NT_HEADERS.OPTIONAL_HEADER.SizeOfImage,
                    self.path,
                )
            )
            ...
            if self.ql.archtype == QL_ARCH.X86:
                ...
                if self.pe.is_dll():
                    ...
                    self.ql.log.debug(
                        "Writing 0x%08X (IMAGE_BASE) to [ESP+4](0x%08X)" % (self.pe_image_address, sp + 0x4)
                    )
            ...
            if not self.is_driver:  # userland program
                ...
                if self.ql.archtype == QL_ARCH.X86:  # Win32
                    ...
                    self.ql.log.debug(
                        "Writing 0x%08X (RegistryPath) to [ESP+8](0x%08X)" % (self.ql.regitry_path_address, sp + 0x8)
                    )
                ...
                self.ql.os.fcall.writeParams(
                    ((POINTER, self.ql.driver_object_address), (POINTER, self.ql.regitry_path_address))
                )
            ...
            if (
                ...
                SecurityCookie_rva = (
                    self.pe.DIRECTORY_ENTRY_LOAD_CONFIG.struct.SecurityCookie - self.pe.OPTIONAL_HEADER.ImageBase
                )
                SecurityCookie_value = default_security_cookie_value = self.ql.mem.read(
                    self.pe_image_address + SecurityCookie_rva, self.ql.pointersize
                )
