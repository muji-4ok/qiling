...
class QlLoaderELF(QlLoader, ELFParse):
    ...
    def load_with_ld(self, stack_addr, load_address=-1, argv=[], env={}):
        ...
        if interp_path != "":
            ...
            self.ql.mem.map(
                self.interp_address, int(interp_mem_size), info=os.path.abspath(self.ql.rootfs + interp_path)
            )
            ...
            for i in interp.parse_segments():
                ...
                if i["p_type"] == "PT_LOAD":
                    self.ql.mem.write(
                        self.interp_address + i["p_vaddr"], interp.getelfdata(i["p_offset"], i["p_filesz"])
                    )
        ...
        self.ql.os.function_hook = FunctionHook(
            self.ql, self.elf_phdr + mem_start, self.elf_phnum, self.elf_phent, load_address, load_address + mem_end
        )
        ...
        for section in _sections:
            ...
            for rel in section.iter_relocations():
                            ...
                            if symbol_name.startswith("sys_"):
                                if self.ql.os.hook_addr % self.ql.pointersize != 0:
                                    self.ql.os.hook_addr = (
                                        int(self.ql.os.hook_addr / self.ql.pointersize) + 1
                                    ) * self.ql.pointersize
                ...
                if describe_reloc_type(rel["r_info_type"], elffile) in ("R_X86_64_32S", "R_X86_64_32"):
                    ...
                    val = ql.unpack16(ql.mem.read(prev_mips_hi16_loc + 2, 2)) << 16 | ql.unpack16(
                        ql.mem.read(loc + 2, 2)
                    )
                    val = rev_reloc_symbols[symbol_name] + val
                    ...
                    raise QlErrorNotImplemented(
                        "Relocation type %s not implemented" % describe_reloc_type(rel["r_info_type"], elffile)
                    )
