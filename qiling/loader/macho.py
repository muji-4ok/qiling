    ...
    def loadDriver(self, stack_addr, loadbase=-1, argv=[], env={}):
        ...
        for relocation in self.macho_file.dysymbol_table.locreloc:
            ...
            self.ql.log.debug(
                ...
                % (loadbase + relocation.address, current_value, seg.name, loadbase + seg.vm_address)
            )
        ...
        self.kernel_local_symbols_detail = self.kernel.symbol_table.details(
            kernel_local_symbols_index, kernel_local_symbols_num, self.kernel.string_table
        )
        ...
        self.kernel_extrn_symbols_detail = self.kernel.symbol_table.details(
            kernel_extrn_symbols_index, kernel_extrn_symbols_num, self.kernel.string_table
        )
        ...
        for relocation in self.macho_file.dysymbol_table.extreloc:
            ...
            if relocation.length == 2 and relocation.rtype == 2:
                if symname not in self.static_symbols:
                    ...
                    self.ql.mem.write(
                        ...
                        struct.pack("<I", self.static_addr + offset - (loadbase + relocation.address + 4)),
                    )
                    ...
                    self.ql.mem.write(
                        loadbase + relocation.address,
                        struct.pack("<I", self.static_symbols[symname] - (loadbase + relocation.address + 4)),
                    )
            ...
            if symname in self.kernel_local_symbols_detail:
                ...
                self.ql.mem.write(
                    loadbase + relocation.address,
                    struct.pack("<Q", self.kernel_local_symbols_detail[symname]["n_value"]),
                )
            elif symname in self.kernel_extrn_symbols_detail:
                ...
                self.ql.mem.write(
                    loadbase + relocation.address,
                    struct.pack("<Q", self.kernel_extrn_symbols_detail[symname]["n_value"]),
                )
            else:
        ...
        if self.IOKit is True:
            ...
            for symbol in self.macho_file.symbol_table.symbols:
                if (
                    symbol.n_type == 0xF
                    and "__const".ljust(16, "\x00") == self.macho_file.sections[symbol.n_sect].name
                ):
                    symname = self.macho_file.string_table[symbol.n_strx]
