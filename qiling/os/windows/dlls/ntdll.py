...
def _QuerySystemInformation(ql, address, params):
    ...
    if siClass == SystemBasicInformation:
        ...
        max_uaddr = {QL_ARCH.X86: 0x7FFEFFFF, QL_ARCH.X8664: 0x7FFFFFFEFFFF}[ql.archtype]
...
def hook_ZwQueryObject(ql, address, params):
    ...
    us = qiling.os.windows.structs.UnicodeString(ql, length=len(string), maxLength=len(string), buffer=string_addr)
...
@winsdkapi(
    cc=STDCALL,
    dllname=dllname,
    replace_params={"ModuleHandle": POINTER, "FunctionName": STRING, "Ordinal": UINT, "FunctionAddress": POINTER},
)
...
#  SIZE_T Size
# );
@winsdkapi(cc=STDCALL, dllname=dllname, replace_params={"HeapHandle": POINTER, "Flags": UINT, "Size": SIZE_T})
