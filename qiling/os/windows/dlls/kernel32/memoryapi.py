...
def hook_VirtualQuery(ql, address, params):
    ...
    ql.mem.write(
        mbi + ql.pointersize * 2, (0x40).to_bytes(length=ql.pointersize, byteorder="little")
    )  # 0x40 = EXECUTE_READ_WRITE
    ql.mem.write(mbi + ql.pointersize * 3, size.to_bytes(length=ql.pointersize, byteorder="little"))
    ql.mem.write(
        mbi + ql.pointersize * 4, (0x1000).to_bytes(length=ql.pointersize, byteorder="little")
    )  # 0x1000 == MEM_COMMIT
    ql.mem.write(
        mbi + ql.pointersize * 5, (0x40).to_bytes(length=ql.pointersize, byteorder="little")
    )  # 0x40 = EXECUTE_READ_WRITE
    ql.mem.write(
        mbi + ql.pointersize * 6, (0x40000).to_bytes(length=ql.pointersize, byteorder="little")
    )  # 0x40000 = MEM_MAPPED
