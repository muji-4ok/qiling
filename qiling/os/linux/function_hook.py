...
class HookFuncMips(HookFunc):
    ...
    def _hook_fuc_exit(self, ql):
        ...
        tmp = self.ql.unpack(
            self.ql.mem.read(self.got + self.load_base + self.gotidx * self.ql.pointersize, self.ql.pointersize)
        )
        if tmp != self.hook_fuc_ptr:
            self.ql.mem.write(
                self.got + self.load_base + self.gotidx * self.ql.pointersize, self.ql.pack(self.hook_fuc_ptr)
            )
            self.ql.mem.write(self.hook_data_ptr, self.ql.pack(tmp))

    def _hook_got(self):
        self.ori_data = self.ql.mem.read(
            self.got + self.load_base + self.gotidx * self.ql.pointersize, self.ql.pointersize
        )
    ...
    def _parse(self):
        ...
        if self.rela != None and self.rela_size != None:
            ...
            if self.ql.archbit == 32:
                self.rela = [
                    ELF32_Rela(
                        rela_buf[_ * self.relaent : (_ + 1) * self.relaent], self.endian, rela_ptr + _ * self.relaent
                    )
        ...
        if self.rel != None and self.rel_size != None:
            ...
            if self.ql.archbit == 32:
                self.rel = [
                    ELF32_Rel(rel_buf[_ * self.relent : (_ + 1) * self.relent], self.endian, rel_ptr + _ * self.relent)
                    for _ in range(self.rel_size // self.relent)
                ]
            elif self.ql.archbit == 64:
                self.rel = [
                    ELF64_Rel(rel_buf[_ * self.relent : (_ + 1) * self.relent], self.endian, rel_ptr + _ * self.relent)
                    for _ in range(self.rel_size // self.relent)
                ]
        ...
        if self.plt_rel != None and self.plt_rel_size != None:
            ...
            if self.plt_rel_type == DT_REL:
                if self.ql.archbit == 32:
                    self.plt_rel = [
                        ELF32_Rel(
                            plt_rel_buf[_ * self.relent : (_ + 1) * self.relent],
                            self.endian,
                ...
                if self.ql.archbit == 32:
                    self.plt_rel = [
                        ELF32_Rela(
                            plt_rel_buf[_ * self.relaent : (_ + 1) * self.relaent],
                            self.endian,
    ...
    def add_function_hook_mips(self, funcname, cb, pos, userdata=None):
        ...
        for symidx in range(self.mips_gotsym, self.mips_symtabno):
            ...
            if tmp_name == funcname.encode():
                ...
                hf = HookFuncMips(
                    self.ql, fn, self.plt_got, symidx - self.mips_gotsym + self.mips_local_gotno, self.load_base
                )
                hf.add_hook(cb, pos, userdata)
