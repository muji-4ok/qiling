...
def ql_syscall_futex(ql, futex_uaddr, futex_op, futex_val, futex_timeout, futex_uaddr2, futex_val3):
    ...
    if futex_op & (FUTEX_PRIVATE_FLAG - 1) == FUTEX_WAIT:
        ...
        #     else:
        #         return True
        regreturn = ql.os.futexm.futex_wait(ql, futex_uaddr, ql.os.thread_management.cur_thread, futex_val)
        ...
        regreturn = ql.os.futexm.futex_wait(ql, futex_uaddr, ql.os.thread_management.cur_thread, futex_val, futex_val3)
        ql.log.debug(
            "futex(%x, %d, %d, %x, %x, %x) = %d"
            % (futex_uaddr, futex_op, futex_val, futex_timeout, futex_uaddr2, futex_val3, regreturn)
        )
