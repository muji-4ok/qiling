...
def ql_syscall_write(ql, write_fd, write_buf, write_count, *args, **kw):
    ...
    try:
        ...
        if hasattr(ql.os.fd[write_fd], "write"):
            ...
            ql.log.warning(
                "write(fd = %d, buf = %x, count = %i) failed due to write_fd" % (write_fd, write_buf, write_count)
            )
...
def ql_syscall_execve(ql, execve_pathname, execve_argv, execve_envp, *args, **kw):
    ...
    ql.log.debug(
        "execve(%s, [%s], [%s])"
        % (pathname, ", ".join(argv), ", ".join([key + "=" + value for key, value in env.items()]))
    )
...
def ql_syscall_getdents(ql, fd, dirp, count, *args, **kw):
    ...
    ql.log.debug(
        ...
        % ("getdents64" if is_64 else "getdents", fd, _ent_count, count, regreturn)
    )
    return regreturn
