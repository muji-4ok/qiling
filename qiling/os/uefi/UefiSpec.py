...
class EFI_ALLOCATE_TYPE(ENUM):
    _members_ = ["AllocateAnyPages", "AllocateMaxAddress", "AllocateAddress", "MaxAllocateType"]
...
class EFI_TIME_CAPABILITIES(STRUCT):
    _fields_ = [("Resolution", UINT32), ("Accuracy", UINT32), ("SetsToZero", BOOLEAN), ("PADDING_0", UINT8 * 3)]
...
class EFI_CAPSULE_HEADER(STRUCT):
    _fields_ = [("CapsuleGuid", EFI_GUID), ("HeaderSize", UINT32), ("Flags", UINT32), ("CapsuleImageSize", UINT32)]
...
EFI_QUERY_CAPSULE_CAPABILITIES = FUNCPTR(
    EFI_STATUS, PTR(PTR(EFI_CAPSULE_HEADER)), UINTN, PTR(UINT64), PTR(EFI_RESET_TYPE)
)
...
EFI_IMAGE_LOAD = FUNCPTR(
    EFI_STATUS, BOOLEAN, EFI_HANDLE, PTR(EFI_DEVICE_PATH_PROTOCOL), PTR(VOID), UINTN, PTR(EFI_HANDLE)
)
...
EFI_OPEN_PROTOCOL_INFORMATION = FUNCPTR(
    EFI_STATUS, EFI_HANDLE, PTR(EFI_GUID), PTR(PTR(EFI_OPEN_PROTOCOL_INFORMATION_ENTRY)), PTR(UINTN)
)
EFI_PROTOCOLS_PER_HANDLE = FUNCPTR(EFI_STATUS, EFI_HANDLE, PTR(PTR(PTR(EFI_GUID))), PTR(UINTN))
EFI_LOCATE_HANDLE_BUFFER = FUNCPTR(
    EFI_STATUS, EFI_LOCATE_SEARCH_TYPE, PTR(EFI_GUID), PTR(VOID), PTR(UINTN), PTR(PTR(EFI_HANDLE))
)
