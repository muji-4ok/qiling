...
class EFI_SMM_SYSTEM_TABLE2(STRUCT):
    ...
    _fields_ = [
        ...
        (
            ...
            FUNCPTR(EFI_STATUS, PTR(EFI_SMM_SYSTEM_TABLE2), PTR(EFI_GUID), PTR(VOID), UINTN),
        ),
        ...
        (
            "SmmAllocatePages",
            FUNCPTR(EFI_STATUS, EFI_ALLOCATE_TYPE, EFI_MEMORY_TYPE, UINTN, PTR(EFI_PHYSICAL_ADDRESS)),
        ),
        ...
        (
            ...
            FUNCPTR(EFI_STATUS, PTR(EFI_HANDLE), PTR(EFI_GUID), EFI_INTERFACE_TYPE, PTR(VOID)),
        ),
        ...
        (
            "SmmLocateHandle",
            FUNCPTR(EFI_STATUS, EFI_LOCATE_SEARCH_TYPE, PTR(EFI_GUID), PTR(VOID), PTR(UINTN), PTR(EFI_HANDLE)),
        ),
...
@dxeapi(params={"Memory": ULONGLONG, "Pages": ULONGLONG})  # EFI_PHYSICAL_ADDRESS  # UINTN
...
@dxeapi(params={"PoolType": INT, "Size": INT, "Buffer": POINTER})  # EFI_MEMORY_TYPE  # UINTN  # PTR(PTR(VOID))
...
@dxeapi(params={"Procedure": POINTER, "CpuNumber": INT, "ProcArguments": POINTER})
...
@dxeapi(params={"Handle": POINTER, "Protocol": GUID, "Interface": POINTER})  # EFI_HANDLE  # PTR(EFI_GUID)  # PTR(VOID)
...
@dxeapi(
    params={"Handle": POINTER, "Protocol": GUID, "Interface": POINTER}  # EFI_HANDLE  # PTR(EFI_GUID)  # PTR(PTR(VOID))
)
...
def hook_SmmRegisterProtocolNotify(ql, address, params):
    ...
    event_dic = {"NotifyFunction": params["Function"], "CallbackArgs": None, "Guid": params["Protocol"], "Set": False}
...
@dxeapi(params={"HandlerType": GUID, "Context": POINTER, "CommBuffer": POINTER, "CommBufferSize": POINTER})
def hook_SmiManage(ql, address, params):
    return EFI_NOT_FOUND


@dxeapi(params={"Handler": POINTER, "HandlerType": GUID, "DispatchHandle": POINTER})
