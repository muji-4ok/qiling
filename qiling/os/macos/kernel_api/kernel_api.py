...
@macos_kernel_api(passthru=True, params={"s1": STRING, "s2": STRING, "n": SIZE_T})
...
@macos_kernel_api(passthru=True, params={"oidp": POINTER, "arg1": POINTER, "arg2": INT, "req": POINTER})
...
@macos_kernel_api(params={"oldValue": POINTER, "newValue": POINTER, "address": POINTER})
...
def hook__ctl_register(ql, address, params):
    ...
    if userctl.ctl_connect.value != 0:
        ...
        ql.os.ev_manager.register(
            userctl.ctl_connect.value, ctl_name, MacOSEventType.EV_CTL_CONNECT, ev_obj=userctl, idx=0
        )
    if userctl.ctl_disconnect.value != 0:
        flag |= 1 << 1
        ql.os.ev_manager.register(
            userctl.ctl_disconnect.value, ctl_name, MacOSEventType.EV_CTL_DISCONNECT, ev_obj=userctl, idx=0
        )
    ...
    if userctl.ctl_setopt.value != 0:
        flag |= 1 << 3
        ql.os.ev_manager.register(
            userctl.ctl_setopt.value, ctl_name, MacOSEventType.EV_CTL_SETOPT, ev_obj=userctl, idx=0
        )
    if userctl.ctl_getopt.value != 0:
        flag |= 1 << 4
        ql.os.ev_manager.register(
            userctl.ctl_getopt.value, ctl_name, MacOSEventType.EV_CTL_GETOPT, ev_obj=userctl, idx=0
        )
    if userctl.ctl_rcvd.value != 0:
        flag |= 1 << 5
        ql.os.ev_manager.register(
            userctl.ctl_rcvd.value, ctl_name, MacOSEventType.EV_CTL_RCVD_FUNC, ev_obj=userctl, idx=0
        )
    if userctl.ctl_send_list.value != 0:
        flag |= 1 << 6
        ql.os.ev_manager.register(
            userctl.ctl_send_list.value, ctl_name, MacOSEventType.EV_CTL_SEND_LIST_FUNC, ev_obj=userctl, idx=0
        )
    if userctl.ctl_bind.value != 0:
        flag |= 1 << 7
        ql.os.ev_manager.register(
            userctl.ctl_bind.value, ctl_name, MacOSEventType.EV_CTL_BIND_FUNC, ev_obj=userctl, idx=0
        )
...
@macos_kernel_api(params={"filter": POINTER, "domain": INT, "type": INT, "protocol": INT})
def hook__sflt_register(ql, address, params):
    ...
    if sf.sf_unregistered != 0:
        flag |= 1 << 0
        ql.os.ev_manager.register(
            sf.sf_unregistered.value, true_name, MacOSEventType.EV_SFLT_UNREGISTERED, protocol=params["protocol"]
        )
    if sf.sf_attach != 0:
        flag |= 1 << 1
        ql.os.ev_manager.register(
            sf.sf_attach.value, true_name, MacOSEventType.EV_SFLT_ATTACH, protocol=params["protocol"]
        )
    if sf.sf_detach != 0:
        flag |= 1 << 2
        ql.os.ev_manager.register(
            sf.sf_detach.value, true_name, MacOSEventType.EV_SFLT_DETACH, protocol=params["protocol"]
        )
    if sf.sf_notify != 0:
        flag |= 1 << 3
        ql.os.ev_manager.register(
            sf.sf_notify.value, true_name, MacOSEventType.EV_SFLT_NOTIFY_CONNECTING, protocol=params["protocol"]
        )
        ql.os.ev_manager.register(
            sf.sf_notify.value, true_name, MacOSEventType.EV_SFLT_NOTIFY_CONNECTED, protocol=params["protocol"]
        )
        ql.os.ev_manager.register(
            sf.sf_notify.value, true_name, MacOSEventType.EV_SFLT_NOTIFY_DISCONNECTING, protocol=params["protocol"]
        )
        ql.os.ev_manager.register(
            sf.sf_notify.value, true_name, MacOSEventType.EV_SFLT_NOTIFY_DISCONNECTED, protocol=params["protocol"]
        )
        ql.os.ev_manager.register(
            sf.sf_notify.value, true_name, MacOSEventType.EV_SFLT_NOTIFY_FLUSH_READ, protocol=params["protocol"]
        )
        ql.os.ev_manager.register(
            sf.sf_notify.value, true_name, MacOSEventType.EV_SFLT_NOTIFY_SHUTDOWN, protocol=params["protocol"]
        )
        ql.os.ev_manager.register(
            sf.sf_notify.value, true_name, MacOSEventType.EV_SFLT_NOTIFY_CANTRECVMORE, protocol=params["protocol"]
        )
        ql.os.ev_manager.register(
            sf.sf_notify.value, true_name, MacOSEventType.EV_SFLT_NOTIFY_CANTSENDMORE, protocol=params["protocol"]
        )
        ql.os.ev_manager.register(
            sf.sf_notify.value, true_name, MacOSEventType.EV_SFLT_NOTIFY_CLOSING, protocol=params["protocol"]
        )
        ql.os.ev_manager.register(
            sf.sf_notify.value, true_name, MacOSEventType.EV_SFLT_NOTIFY_BOUND, protocol=params["protocol"]
        )
    if sf.sf_getpeername != 0:
        flag |= 1 << 4
        ql.os.ev_manager.register(
            sf.sf_getpeername.value, true_name, MacOSEventType.EV_SFLT_GETPEERNAME, protocol=params["protocol"]
        )
    if sf.sf_getsockname != 0:
        flag |= 1 << 5
        ql.os.ev_manager.register(
            sf.sf_getsockname.value, true_name, MacOSEventType.EV_SFLT_GETSOCKNAME, protocol=params["protocol"]
        )
    if sf.sf_data_in != 0:
        flag |= 1 << 6
        ql.os.ev_manager.register(
            sf.sf_data_in.value, true_name, MacOSEventType.EV_SFLT_DATA_IN, protocol=params["protocol"]
        )
    if sf.sf_data_out != 0:
        flag |= 1 << 7
        ql.os.ev_manager.register(
            sf.sf_data_out.value, true_name, MacOSEventType.EV_SFLT_DATA_OUT, protocol=params["protocol"]
        )
    if sf.sf_connect_in != 0:
        flag |= 1 << 8
        ql.os.ev_manager.register(
            sf.sf_connect_in.value, true_name, MacOSEventType.EV_SFLT_CONNECT_IN, protocol=params["protocol"]
        )
    if sf.sf_connect_out != 0:
        flag |= 1 << 9
        ql.os.ev_manager.register(
            sf.sf_connect_out.value, true_name, MacOSEventType.EV_SFLT_CONNECT_OUT, protocol=params["protocol"]
        )
    if sf.sf_bind != 0:
        flag |= 1 << 10
        ql.os.ev_manager.register(
            sf.sf_bind.value, true_name, MacOSEventType.EV_SFLT_BIND, protocol=params["protocol"]
        )
    if sf.sf_setoption != 0:
        flag |= 1 << 11
        ql.os.ev_manager.register(
            sf.sf_setoption.value, true_name, MacOSEventType.EV_SFLT_SETOPTION, protocol=params["protocol"]
        )
    if sf.sf_getoption != 0:
        flag |= 1 << 12
        ql.os.ev_manager.register(
            sf.sf_getoption.value, true_name, MacOSEventType.EV_SFLT_GETOPTION, protocol=params["protocol"]
        )
    if sf.sf_listen != 0:
        flag |= 1 << 13
        ql.os.ev_manager.register(
            sf.sf_listen.value, true_name, MacOSEventType.EV_SFLT_LISTEN, protocol=params["protocol"]
        )
    if sf.sf_ioctl != 0:
        flag |= 1 << 14
        ql.os.ev_manager.register(
            sf.sf_ioctl.value, true_name, MacOSEventType.EV_SFLT_IOCTL, protocol=params["protocol"]
        )
...
def hook__mac_policy_register(ql, address, params):
    ...
    for i in range(0, NUM_EVENT_MAC_POLICY):
        ...
        if func != 0:
            ql.os.ev_manager.register(
                func, true_name, MACPolicy_EventType[MACPolicy_EventType(i + base_event_MAC).name]
            )
            ql.log.debug(
...
def hook__getattrlistbulk(ql, address, params):
    ...
    for path in dirfd.iterdir():
        ...
        code = gen_stub_code(
            ql,
            [0, uio.base, getattrlistbulk_args.alist, getattrlistbulk_args.options, vap.base, 0, params["p"]],
            vfs_attr_pack,
        )
