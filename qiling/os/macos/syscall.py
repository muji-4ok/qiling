...
def ql_syscall_kernelrpc_mach_vm_allocate_trap(ql, port, addr, size, flags, *args, **kw):
    ql.log.debug(
        "[mach] mach vm allocate trap(port: 0x%x, addr: 0x%x, size: 0x%x, flags: 0x%x" % (port, addr, size, flags)
    )
...
def ql_syscall_pread(ql, fd, buf, nbyte, offset, *args, **kw):
    ql.log.debug("pread(fd: 0x%x, buf: 0x%x, nbyte: 0x%x, offset: 0x%x)" % (fd, buf, nbyte, offset))
...
def ql_syscall_bsdthread_register(
    ql, threadstart, wqthread, flags, stack_addr_hint, targetconc_ptr, dispatchqueue_offset
):
    set_eflags_cf(ql, 0x0)
...
def ql_syscall_abort_with_payload(
    ql, reason_namespace, reason_code, payload, payload_size, reason_string, reason_flags
):
    ql.log.debug(
