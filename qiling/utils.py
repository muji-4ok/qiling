...
class QilingPlainFormatter(logging.Formatter):
    ...
    def get_level(self, record: LogRecord) -> str:
        LEVEL_NAME = {"WARNING": "[!]", "INFO": "[=]", "DEBUG": "[+]", "CRITICAL": "[x]", "ERROR": "[x]"}
...
def ql_pe_parse_emu_env(path):
    ...
    if arch:
        if (
            pe.OPTIONAL_HEADER.Subsystem >= pefile.SUBSYSTEM_TYPE["IMAGE_SUBSYSTEM_EFI_APPLICATION"]
            and pe.OPTIONAL_HEADER.Subsystem <= pefile.SUBSYSTEM_TYPE["IMAGE_SUBSYSTEM_EFI_ROM"]
        ):
            ostype = QL_OS.UEFI
        else:
...
def debugger_setup(debugger, ql):
    ...
    debugsession = ql_get_module_function(
        f"qiling.debugger.{remotedebugsrv}.{remotedebugsrv}", f"Ql{str.capitalize(remotedebugsrv)}"
    )
...
def profile_setup(ostype, profile, ql):
    ...
    os_profile = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "profiles", ostype_convert_str(ostype) + ".ql"
    )
